...
ざっくりいうと、「(自分がメンテナンスしているわけではないけれども)まだ誰も直していない放置されているバグを探してフィードバックしたい」です。

bugs.debian.org や qa.debian.org は自分がメンテナンスをしているパッケージに関してはよくサポートされています。
しかし、直接関わっているわけではない放置されているバグを探したりフィードバックしたりするにはあまり向いていないのです。
...
1vCPU 1GBメモリのインスタンスだと、使っているのが自分一人とはいえ、メモリがやっぱり厳しいこととか、
...
1vCPUのインスタンスなので、負荷を抑えつつどこまでできるかはまだ未知数ですが。

### まとめ

fabre.debian.net の開発の進め方と運用、得た学びの話について書きました。
